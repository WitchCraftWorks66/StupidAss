...
--如果没有polyb，请注释掉第852到第862行。（以后我会发出相应的文件，可以留意一下更新）
--转载请注明作者和出处！
...
function voronoi_cell(final_tri,p,x0,y0,w,h)
	...
	for i=1,#p do
		local temp,vor_cell,circum={},{},{} local edges
		for i1=1,#final_tri do
			if check(final_tri[i1],p[i]) then
				temp[#temp+1]=final_tri[i1]
			end
		end
